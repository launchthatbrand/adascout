...
export const extract = action({
  ...
  handler: async (_ctx: any, args: any) => {
    ...
    try {
      ...
      if (ownSession) await api.endSession(sessionId, config);
  ...
  handler: async (_ctx: any, args: any) => {
    ...
    try {
      ...
      const result = await api.agentExecute(
        sessionId,
        { cua: args.options?.cua, systemPrompt: args.options?.systemPrompt },
        { instruction: args.instruction, maxSteps: args.options?.maxSteps },
        config,
      );
      if (ownSession) await api.endSession(sessionId, config);
      return result.result;
